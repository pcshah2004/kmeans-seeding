/**
 * Five-Method K-means++ Benchmark with PRONE (boosted)
 *
 ...
 * 4. PRONE (boosted, α=0.001) - PRONE + sensitivity sampling coreset + weighted k-means++
 * 5. PRONE (boosted, α=0.01) - PRONE + sensitivity sampling coreset + weighted k-means++
 * 6. PRONE (boosted, α=0.1) - PRONE + sensitivity sampling coreset + weighted k-means++
 *
...
int main(int argc, char** argv) {
...
#endif
    std::cout << "  4. PRONE (boosted, α=0.001)\n";
    std::cout << "  5. PRONE (boosted, α=0.01)\n";
    std::cout << "  6. PRONE (boosted, α=0.1)\n";
    std::cout << "========================================\n\n";
    ...
    std::ofstream outfile("../experiments/benchmark_results_prone_boosted.csv");
    if (!outfile.is_open()) {
    ...
    for (int k : k_values) {
        ...
        // 4. PRONE (boosted, α=0.001) - Very small coreset
        {
            std::cout << "4. PRONE (boosted, α=0.001)\n";
            std::cout << "   Running..." << std::flush;
            ...
            auto result = prone_boosted::run_prone_boosted(data, n, d, k, 0.001, random_seed);
            ...
            outfile << "PRONE (boosted α=0.001)," << k << "," << runtime << "," << cost << "\n";
            outfile.flush();
        }

        // 5. PRONE (boosted, α=0.01) - Medium coreset
        {
            std::cout << "5. PRONE (boosted, α=0.01)\n";
            std::cout << "   Running..." << std::flush;
            ...
            auto result = prone_boosted::run_prone_boosted(data, n, d, k, 0.01, random_seed);
            ...
            outfile << "PRONE (boosted α=0.01)," << k << "," << runtime << "," << cost << "\n";
            outfile.flush();
        }

        // 6. PRONE (boosted, α=0.1) - Large coreset
        {
            std::cout << "6. PRONE (boosted, α=0.1)\n";
            std::cout << "   Running..." << std::flush;
            ...
            auto result = prone_boosted::run_prone_boosted(data, n, d, k, 0.1, random_seed);
            ...
            outfile << "PRONE (boosted α=0.1)," << k << "," << runtime << "," << cost << "\n";
            outfile.flush();
        }
    }
    ...
    std::cout << "Results written to: ../experiments/benchmark_results_prone_boosted.csv\n";
